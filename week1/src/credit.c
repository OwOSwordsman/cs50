...
int main(void) {
    ...
    // printf("%i\n", isValid); // LOG
    if (!isValid) { // print invalid if card is not valid and end program
        printf("INVALID\n");
        return 0;
    }
...
bool checkCardNum(long cardNum, int length) {
    ...
    if (length != 15 && length != 16 && length != 13) return false;

    // sums digits and make sure it is a multiple of 10
    int digitSum = sumEveryOtherDigit(cardNum);
    // printf("%i\n", digitSum); // LOG
    if (!(digitSum % 10 == 0 && getNumLength(cardNum))) return false;
    return true;
}
...
int sumEveryOtherDigit(long cardNum) {
    int multipliedSum = 0;
    int remainingSum = 0;
    while (cardNum / 10 != 0) { // check make sure there are at least 2 digits left
