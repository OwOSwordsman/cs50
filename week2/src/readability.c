...
int getLetterCt(string text) {
    ...
    for (int i = 0, l = strlen(text); i < l; i++) {
        ...
        if (tempChar >= 0 && tempChar <= 26) letterCt++;    // add to var when tempChar is within range
    }
    return letterCt;
}
...
int getWordCt(string text) {
    ...
    int wordCt = 0;
    for (int i = 0, l = strlen(text); i < l; i++) {
        if (text[i] == 32) wordCt++;    // add to word var for every blank space
    }
...
int getSentenceCt(string text) {
    ...
    int sentenceCt = 0;
    for (int i = 0, l = strlen(text); i < l; i++) {
        if (text[i] == '!' || text[i] == '?' || text[i] == '.') sentenceCt++;
    }
    return sentenceCt;
}
